...
export class ExcelReporter {
  ...
  async generateReport(resultsData) {
    ...
    workbook.creator = 'Selenium E2E Test Automation Framework';
    ...
    const resultsSheet = workbook.addWorksheet('Test Results Detail', {
      views: [{ showGridLines: true }]
    });
  ...
  buildSummarySheet(sheet, summary) {
    ...
    titleCell.value = 'IMAGE AUTHENTICITY APP - E2E SELENIUM TEST REPORT';
    ...
    subCell.value = `Generated on: ${new Date().toLocaleString()} | Framework: Node.js + Selenium WebDriver`;
    subCell.font = { name: 'Segoe UI', size: 10, italic: true, color: { argb: '475569' } };
    subCell.alignment = { horizontal: 'center', vertical: 'middle' };

    // KPI Summary Cards (Row 6 to 9)
    const kpis = [
      { label: 'Total Tests Executed', value: summary.totalTests, color: '3B82F6' },
    ...
    // Environment & Run Details Table
    sheet.mergeCells('B10:E10');
    const envHeader = sheet.getCell('B10');
    envHeader.value = 'Execution Context & Environment Info';
    ...
    const details = [
      ['Target Web Application URL', summary.baseUrl || 'http://localhost:5173'],
      ['Automation Driver', 'Selenium WebDriver (Node.js)'],
      ['Target Browser', summary.browser || 'Chrome (Headless)'],
      ['Total Execution Duration', `${(summary.durationMs / 1000).toFixed(2)} seconds`],
  ...
  buildResultsSheet(sheet, testCases) {
    sheet.columns = [
      ...
      { header: 'Module / Category', key: 'category', width: 24 },
      { header: 'Test Case Name', key: 'name', width: 35 },
      { header: 'Tested Route / Feature', key: 'route', width: 24 },
    ...
    testCases.forEach((tc, idx) => {
      ...
      // Center align ID, status, duration, timestamp
      ...
      if (idx % 2 === 1) {
        row.eachCell((cell, colNumber) => {
          if (colNumber !== 5) { // Skip status column custom fill
  ...
  buildModuleSheet(sheet, moduleMetrics) {
    sheet.columns = [
      { header: '#', key: 'id', width: 6 },
      { header: 'Functional Area / Module', key: 'module', width: 32 },
